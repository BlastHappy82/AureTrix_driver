...
4.  **Layout Configuration System:** Centralized management of absolute positioning data for accurate rendering of diverse keyboard layouts. Uses a 3-tier priority system for layout loading: (1) **sharedLayout.ts** - Community-contributed layouts with precise measurements checked first by productName (e.g., Slice75 HE), (2) **IndexedDB** - User-created custom layouts stored locally via LayoutStorageService, and (3) **layoutMap** - Standard layouts by keyCount (61, 67, 68, 80, 82, 84, 87 key). This prioritization ensures shared community layouts override defaults while preserving user customization capability.
