...
8.  **Hardware Settings Synchronization:** Implements a delayed synchronization pattern for reading hardware settings (polling rate, system mode) on connection. Uses a managed 500ms timeout to allow SDK initialization before querying settings, with proper timeout cleanup on disconnection and connection state validation before execution to prevent stale callbacks from running after device disconnection.
