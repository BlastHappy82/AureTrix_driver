...
7.  **Custom Layout Creator:** A full-page visual layout builder (`/layout-creator`) with a compact UI for creating custom keyboard layouts. Users specify row counts (Row1=15, Row2=15, etc.) and row gaps to generate a virtual keyboard, then click keys to edit sizes (1u, 1.25u, etc.) and per-key gaps. Eliminates the need for column gaps by using gap-after values on individual keys. Includes automatic controller emulation metadata detection via `getAxisList()` SDK call. Layouts are stored locally in IndexedDB and can be exported as JSON or submitted to GitHub for community sharing.
